...
class HeapNode<S extends Comparable<S>, T extends Comparable<T>>{
    ...
    public int compareHeapNode(S new_value1, T new_value2) {
      //int lets = this.value2.compareTo(new_value2);
      
      if(this.value2.compareTo(new_value2) > 0){
        return 1;
      }
...
{    
    ...
    public int compareAVLNode(S new_value1, T new_value2) {
      //int check = this.value1.compareTo(new_value1);
      if(this.value1.compareTo(new_value1) > 0){
        return 1;
      }
...
public class GPADatabase {
	MaxHeap<String, Double> this_heap;
	AVLTree<String, Double> this_avltree;
...
//public int sizehelper(AVLNode a){
//  if(a == null){
//    return 0;
//  }
//  return 1 + sizehelper(a.l_child) + sizehelper(a.r_child);
//}
    ...
    this_heap.add(new HeapNode<String,Double>(student_name,gpa));
    ...
    ArrayList<HeapNode<String,Double>> nnn = new ArrayList<HeapNode<String,Double>>();
    int count = this_avltree.countNodes();
    if (count == 0){
      return gPAS;
   } else if (count <= k){
      count = k;
      for (int i = 0; i<k; i++){
        HeapNode<String,Double> rrr = this.this_heap.poll();
        gPAS.add(rrr.value1.toString());
        nnn.add(rrr);
    }
     for(int i = 0; i<k; i++){
      this.this_avltree.deleteNode(nnn.get(i).value1, nnn.get(i).value2);
    }
      return gPAS;
  } else{
      for (int i = 0; i<k; i++){
...
//}
    ...
    AVLNode<String,Double> Collect;
    Collect = this_avltree.search(student_name, null);
		if (Collect == null){
      return null;
    } else{
      return (double)Collect.value2;
    }
	}
